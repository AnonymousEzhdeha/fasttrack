...
def main(exp, args):
    ...
    # --- BDD100K video inference (real multi-frame tracking) ---
    if args.demo == "bdd100k":
        video_dir = r"bdd100k/bdd100k/videos/val"
        video_files = sorted([
        ...
        logger.info(f"Found {len(video_files)} BDD100K videos.")

        # Loop over each BDD100K video
        for vid_idx, vid_path in enumerate(video_files):
            logger.info(f"Processing BDD100K video {vid_idx+1}/{len(video_files)}: {vid_path}")
            ...
            timestamp = time.strftime("%Y_%m_%d_%H_%M_%S", current_time)
            save_folder = osp.join(vis_folder, f"bdd100k_{timestamp}")
            os.makedirs(save_folder, exist_ok=True)

            save_path = osp.join(save_folder, osp.basename(vid_path))
            vid_writer = cv2.VideoWriter(
                save_path, cv2.VideoWriter_fourcc(*"mp4v"), fps, (width, height)
            )

            # Init tracker (fresh tracker per video)
            if args.occ:
                config = load_or_init_config(args)
                tracker = Fasttracker(args, config, frame_rate=fps)
            else:
            ...
            while True:
                ...
                if outputs[0] is not None:
                    ...
                    timer.toc()
                    tracked_img = plot_tracking(
                        img_info["raw_img"], online_tlwhs, online_ids,
                        frame_id=frame_id+1, fps=1./timer.average_time,
                        roi_points=getattr(tracker, "roi_points", [])
                    )
                else:
                    timer.toc()
                    tracked_img = img_info["raw_img"]

                if args.save_result:
                    vid_writer.write(tracked_img)
            ...
            # Save MOT results for this video
            result_txt = osp.join(save_folder, osp.basename(vid_path) + ".txt")
            with open(result_txt, "w") as f:
                f.writelines(results)
            logger.info(f"Saved tracking results to {result_txt}")
